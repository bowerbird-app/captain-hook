  ...
  class OutgoingJob < ApplicationJob
    ...
    def perform(event_id)
      ...
      if endpoint_config.circuit_breaker_enabled?
        ...
        rescue CaptainHook::Services::CircuitBreaker::CircuitOpenError
      ...
      begin
        ...
        if OutgoingEvent.success_response?(response_code)
          ...
          circuit_breaker.record_success(event.target_url) if endpoint_config.circuit_breaker_enabled?
          ...
          event.mark_failed!(error_message, response_code: response_code, response_body: response_body,
                                            response_time_ms: response_time_ms)
          ...
          if event.max_attempts_reached?(endpoint_config.max_attempts)
            ...
            Instrumentation.outgoing_failed(event, error: StandardError.new(error_message),
                                                   response_code: response_code)
          else
    ...
    def validate_url_safety!(uri)
      ...
      raise ArgumentError, "URL must use HTTP or HTTPS scheme" unless %w[http https].include?(uri.scheme)
