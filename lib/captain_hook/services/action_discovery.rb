    ...
    class ActionDiscovery < BaseService
      ...
      def extract_action_details(action_class)
        unless action_class.respond_to?(:details)
          ...
          Rails.logger.warn "    Add: def self.details; { event_type: 'your.event', " \
                            "priority: 100, async: true, max_attempts: 5 }; end"
          return nil
        end
        ...
        if parts.length >= 3
          ...
          if defined?(Gem)
            ...
            if gem_spec
              ...
              load_gem_action_files(spec, provider)
      ...
      def self.load_gem_action_files(spec, provider)
        action_file_pattern = File.join(spec.full_gem_path, "captain_hook", provider, "actions", "**", "*.rb")
        Dir.glob(action_file_pattern).each do |file|
          require file
        rescue StandardError
          nil
        end
      end
      private_class_method :load_gem_action_files
